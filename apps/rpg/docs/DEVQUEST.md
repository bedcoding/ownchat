...
치환은 **화면에 그릴 때만** 일어난다 (`lib/brand.ts` 의 `brandWork`, Runner 안에서 호출).
저작 도구가 들고 있는 원본은 토큰을 유지하므로, 관리자가 편집·발행한 JSON 에 실명이 섞이지 않는다.

> 저장소를 클론한 사람은 `{PLATFORM}` 같은 깨진 문자열이 아니라 **완결된 게임**을 본다.
> 토큰을 그대로 노출하면 그 장면들이 전부 읽히지 않기 때문이다.
