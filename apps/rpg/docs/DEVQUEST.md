...
> **가상 조직은 가해자가 아니라 같이 당한 쪽이다.**

사람을 밀어낸 것은 조직의 결정이 아니다. 외부에서 들어온 정체 불명의 시스템이고, **그 출처는 끝까지 밝히지 않는다.** 해고 통보에는 보내는 사람이 있지만 이것에는 없다 — 4화의 "해당 계정은 현재 필요하지 않습니다"가 그 지점이다.

이 선을 지키면 특정 조직을 연상시키지 않으면서 할 말은 다 한다. 전부 가상 세계의 사건으로 읽히는 것이 핵심이다.
...
가상 개발 조직을 배경으로 하며 **실재하는 조직이나 서비스 명칭을 사용하지 않는다.**
...
# NEXT_PUBLIC_PLATFORM_NAME=가상플랫폼
```
...
1. **인물 실루엣 시트**를 먼저 1장 — 주인공·김선임·채용봇·클라이언트·윤편집 5종. 실루엣이라 한 장에 다 담긴다.
   무한루프와 연재기는 사람 형태가 아니라 장면 아트에서 직접 다룬다
...
- **다른 직군**: 스탯 3개와 판정 문구만 갈면 기획·디자인 버전이 된다
